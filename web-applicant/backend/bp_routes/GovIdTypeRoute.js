...
import {
    ...
    getAllGovIdTypes,
    deleteGovIdType
} from "../bp_controllers/GovIdTypeController.js";
...
/**
 * @route   DELETE /api/gov-id-type/:id
 * @desc    Delete government ID type by ID
 * @access  Public
 */
router.delete("/:id", async (req, res) => {
    try {
        const deleted = await deleteGovIdType(req.params.id);
        
        if (!deleted) {
            return res.status(404).json({
                success: false,
                message: "Government ID type not found or delete failed"
            });
        }

        res.status(200).json({
            success: true,
            message: "Government ID type deleted successfully"
        });
    } catch (error) {
        res.status(500).json({
            success: false,
