...
import {
    ...
    getOccupancyUseTypesByGroupId
} from "../bp_controllers/OccupancyUseTypeController.js";
...
router.get("/", async (req, res) => {
    try {
        const occupancyUseTypes = await getAllOccupancyUseTypes();
        
        res.status(200).json({
            success: true,
            data: occupancyUseTypes
        });
    } catch (error) {
        res.status(500).json({
            success: false,
...
router.get("/group/:groupId", async (req, res) => {
    try {
        const occupancyUseTypes = await getOccupancyUseTypesByGroupId(req.params.groupId);
...
router.get("/:id", async (req, res) => {
    try {
        const occupancyUseType = await getOccupancyUseTypeById(req.params.id);
