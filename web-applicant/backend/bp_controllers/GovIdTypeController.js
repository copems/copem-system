 ...
 * @returns {Promise<string>} The newly created git_id
 */
export const saveGovIdType = async (govIdTypeData) => {
    ...
    try {
        ...
        console.log("DB Result structure (GovIdType):", { rows, rowsType: Array.isArray(rows), rowsLength: rows?.length });
        
        if (!Array.isArray(rows) || rows.length === 0) {
            if (rows && typeof rows === 'object' && !Array.isArray(rows)) {
                const gitId = rows.git_id;
                if (gitId) return gitId;
            }
            throw new Error("Failed to retrieve government ID type ID from database. Ensure stored procedure returns the ID.");
        }
        
        const firstRow = rows[0];
        if (!firstRow || !firstRow.git_id) {
            throw new Error("Failed to retrieve government ID type ID from database. Returned data is invalid.");
        }
        return firstRow.git_id;
    } catch (error) {
 ...
 * @param {string} gitId - The government ID type ID
...
/**
 * Delete government ID type by ID
 * @param {string} gitId - The government ID type ID
 * @returns {Promise<boolean>} True if delete was successful
 */
export const deleteGovIdType = async (gitId) => {
    try {
        await pool.query(
            `CALL sp_DeleteGovIdType(?)`,
            [gitId]
        );

        return true;
    } catch (error) {
        throw new Error(`Error deleting government ID type: ${error.message}`);
    }
};
